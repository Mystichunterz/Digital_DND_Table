import "../styles/pages/background.scss";

const Background = () => {
  return (
    <div className="background">
      <h1>Background</h1>
      <div className="grid">
        <section className="div4">
          <h2>Backstory</h2>
          <p>
            Lorem ipsum dolor sit amet consectetur adipisicing elit. Fuga, vitae
            rerum eligendi, est qui repellat quia modi dolorem pariatur soluta
            nesciunt aliquid. Quo fugit, voluptatum unde recusandae nisi quos
            libero.Lorem ipsum dolor sit amet consectetur adipisicing elit.
            rerum eligendi, est qui repellat quia modi dolorem pariatur soluta
            nesciunt aliquid. Quo fugit, voluptatum unde recusandae nisi quos
            libero.Lorem ipsum dolor sit amet consectetur adipisicing elit.
            Lorem ipsum dolor sit amet consectetur adipisicing elit. Fuga, vitae
            rerum eligendi, est qui repellat quia modi dolorem pariatur soluta
            nesciunt aliquid. Quo fugit, voluptatum unde recusandae nisi quos
            libero.Lorem ipsum dolor sit amet consectetur adipisicing elit.
            rerum eligendi, est qui repellat quia modi dolorem pariatur soluta
            nesciunt aliquid. Quo fugit, voluptatum unde recusandae nisi quos
            libero.Lorem ipsum dolor sit amet consectetur adipisicing elit.
            Lorem ipsum dolor sit amet consectetur adipisicing elit. Fuga, vitae
            rerum eligendi, est qui repellat quia modi dolorem pariatur soluta
            nesciunt aliquid. Quo fugit, voluptatum unde recusandae nisi quos
            libero.Lorem ipsum dolor sit amet consectetur adipisicing elit.
            rerum eligendi, est qui repellat quia modi dolorem pariatur soluta
            nesciunt aliquid. Quo fugit, voluptatum unde recusandae nisi quos
            libero.Lorem ipsum dolor sit amet consectetur adipisicing elit.
          </p>
        </section>
        <section className="div2">
          <h2>Personality Traits</h2>
          <p>Is a master coder 10x developer even</p>
        </section>
        <section className="div3">
          <h2>Ideals</h2>
          <p>
            <strong>Bland Food</strong> believes &quot;Mystic Chicken&quot; to
            be an edible dish.
          </p>
        </section>
        <section className="div5">
          <h2>Bonds</h2>
          <ul>
            <li>Lav - Party Member</li>
            <li>Midnight - New Party Member</li>
            <li>Iridith - Queen of the Elves</li>
          </ul>
        </section>
        <section className="div1">
          <h2>Flaws</h2>
          <p>Allergic to Milk🍼 </p>
        </section>
      </div>
    </div>
  );
};
export default Background;
